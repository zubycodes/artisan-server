...
GROUP BY c.id, c.name
  ...
  update(id, technique) {
    const { name, category_Id, isActive } = technique;
    return dbAsync.run(
      'UPDATE techniques SET name = ?, category_Id = ?, isActive = ? WHERE id = ?',
      [name, category_Id, isActive, id]
    );
  },

  delete(id) {
    return dbAsync.run('DELETE FROM techniques WHERE id = ?', [id]);
  }
};
...
module.exports = (dependencies) => {
  ...
  const handlers = {
    getAll: createHandler(async (req, res) => {
      const routeLogger = logger.child({ route: 'techniques', handler: 'getAll' });
    ...
    create: createHandler(async (req, res) => {
      const routeLogger = logger.child({ route: 'techniques', handler: 'create' });
      ...
      try {
        ...
        res.write(`data: ${JSON.stringify({ status: 'complete', id: lastID, message: 'Technique created successfully' })}\n\n`);
        res.status(201).end();
      } catch (error) {
    ...
    update: createHandler(async (req, res) => {
      const routeLogger = logger.child({ route: 'techniques', handler: 'update' });
      routeLogger.info({ id: req.params.id, body: req.body }, 'Received update technique request');
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
      res.flushHeaders();
      ...
      try {
        ...
        if (changes === 0) {
          ...
          res.write(`data: ${JSON.stringify({ status: 'error', message: 'Technique not found' })}\n\n`);
          return res.status(404).end();
        }

        res.write(`data: ${JSON.stringify({ status: 'complete', id: parseInt(req.params.id), message: 'Technique updated successfully' })}\n\n`);
        res.status(200).end();
      } catch (error) {
    ...
    delete: createHandler(async (req, res) => {
      const routeLogger = logger.child({ route: 'techniques', handler: 'delete' });
      routeLogger.info({ id: req.params.id }, 'Received delete technique request');
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
      res.flushHeaders();
      ...
      try {
        ...
        res.write(`data: ${JSON.stringify({ status: 'complete', message: 'Technique deleted successfully' })}\n\n`);
        res.status(200).end();
      } catch (error) {
