...
const geoLevelOps = {
  ...
  create({ code, name }) {
    return dbAsync.run(
      'INSERT INTO geo_level (code, name) VALUES (?, ?)',
      [code, name]
    );
  },
  ...
  update(id, { code, name }) {
    return dbAsync.run(
      'UPDATE geo_level SET code = ?, name = ? WHERE id = ?',
      [code, name, id]
    );
  },
...
module.exports = (dependencies) => {
  ...
  const handlers = {
    ...
    getAll: createHandler(async (req, res) => {
      const routeLogger = logger.child({ route: 'geo_level', handler: 'getAll' });
      routeLogger.info('Received get all geo levels request');
      try {
        const geoLevels = await geoLevelOps.getAll();
        res.json(geoLevels);
      } catch (error) {
    ...
    create: createHandler(async (req, res) => {
      const routeLogger = logger.child({ route: 'geo_level', handler: 'create' });
      ...
      try {
        ...
        res.write(`data: ${JSON.stringify({ status: 'complete', id: lastID, message: 'Geo level created successfully' })}\n\n`);
        res.status(201).end();
      } catch (error) {
    ...
    update: createHandler(async (req, res) => {
      const routeLogger = logger.child({ route: 'geo_level', handler: 'update' });
      routeLogger.info({ id: req.params.id, body: req.body }, 'Received update geo level request');
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
      res.flushHeaders();
      ...
      try {
        ...
        if (changes === 0) {
          ...
          res.write(`data: ${JSON.stringify({ status: 'error', message: 'Geo level not found' })}\n\n`);
          return res.status(404).end();
        }

        res.write(`data: ${JSON.stringify({ status: 'complete', id: parseInt(id), message: 'Geo level updated successfully' })}\n\n`);
        res.status(200).end();
      } catch (error) {
    ...
    delete: createHandler(async (req, res) => {
      const routeLogger = logger.child({ route: 'geo_level', handler: 'delete' });
      routeLogger.info({ id: req.params.id }, 'Received delete geo level request');
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
      res.flushHeaders();
      ...
      try {
        ...
        res.write(`data: ${JSON.stringify({ status: 'complete', message: 'Geo level deleted successfully' })}\n\n`);
        res.status(200).end();
      } catch (error) {
