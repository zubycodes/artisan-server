  ...
  getDashboardData(filters = {}) {
    ...
    let query = `
      SELECT
        ...
        (SELECT COUNT(*) FROM artisansView a WHERE a.isActive = 1 {WHERE_CLAUSE} AND a.created_at >= date('now', 'start of month')) AS new_registrations_this_month,
        (SELECT COUNT(*) FROM artisansView a WHERE a.isActive = 1 {WHERE_CLAUSE} 
        AND a.created_at >= date('now', 'start of month', '-1 month') 
        AND a.created_at < date('now', 'start of month')) AS new_registrations_last_month
    `;
    const params = [];
    ...
    whereClause = addFilterCondition(education, 'education_name', params, whereClause);
    ...
    getAllChartData: createHandler(async (req, res) => {
      ...
      try {
        ...
        const [
          ...
          craftByEmploymentType,
          categoryByEmploymentType,
          ...
          chartOps.getDistribution('craft', filters),
          chartOps.getDistribution('category', filters),
          ...
          chartOps.getAverageIncomeByGroup('craft', filters),
          chartOps.getAverageIncomeByGroup('category', filters),
          ...
          chartOps.getDistributionByEmploymentType('craft', filters),
          chartOps.getDistributionByEmploymentType('category', filters),
